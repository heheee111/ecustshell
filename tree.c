...
//使用深度优先的思路实现对文件树的输出
void tree_dfs(char *_path, int target_deep, int this_deep)
{
    //调整路径防越界
    ...
    while (1)
    {
        ...
        if ((items = readdir(dir)) == NULL)
        {
            //将tree_flag[this_deep]置0
            tree_flag[this_deep] = 0;

            //选择最后一项形式输出
            tree_print2(this_deep);
            printf("%s", c_name);
        }
        else
        {
            //选择非最后一项形式输出
            tree_print1(this_deep);
            printf("%s", c_name);
        }
        ...
        if (S_ISDIR(the_stat.st_mode))
        {
            ...
            else
            {
                ...
                //同时调用dfs函数，层数加一
                tree_dfs(the_path, target_deep, this_deep + 1);
            }
        }
...
int tree(int argc, char *argv[])
{
    ...
    if (argc != 2 && argc != 3)
    {
        ...
        printf("Usage1: tree <path>\n");
        printf("Usage2: tree <path> <depth>\n\n");
