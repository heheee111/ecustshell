...
void Init()
{
    /* Reap any zombie processes left by background execution */
    int status;
...
int callCommandWithPipe(int left, int right)
{
    ...
    if (left_pid == 0) {
        ...
        close(fds[1]);
        int r = callCommandWithRedi(left, pipeIdx);
        exit(r);
    }

    /* --- Fork right side (reader) --- */
    pid_t right_pid = fork();
    if (right_pid == -1) {
        close(fds[0]);
        close(fds[1]);
        waitpid(left_pid, NULL, 0);   /* reap left child before returning */
        return ERROR_FORK;
    }
    if (right_pid == 0) {
        /* Child: redirect stdin ← pipe read end */
        close(fds[1]);
        dup2(fds[0], STDIN_FILENO);
        close(fds[0]);
        int r = callCommandWithPipe(pipeIdx + 1, right);  /* recurse for multi-pipe */
        exit(r);
    }
    ...
    int left_exit  = WIFEXITED(lstatus)  ? WEXITSTATUS(lstatus)  : RESULT_NORMAL;
    int right_exit = WIFEXITED(rstatus) ? WEXITSTATUS(rstatus) : RESULT_NORMAL;

    /* Prefer a non-zero exit from either side */
    if (right_exit != RESULT_NORMAL) result = right_exit;
    else if (left_exit != RESULT_NORMAL) result = left_exit;

    return result;
}

int callCommand(int commandNum, int background)
{
    /* check alias */
    ...
    if (pid == 0) {
        ...
        if (background) {
            /* Print child PID and return immediately — do NOT wait */
            printf("[%d]\n", pid);
    ...
    while (1) {
        ...
        if (buf[0] == '!') {
            ...
            if (buf[1] == '!' && (buf[2] == '\n' || buf[2] == '\0')) {
                ...
                strcpy(expanded, history[cmd_cnt - 1]);
                found = 1;
            }
            /* !?string? — fuzzy match */
            else if (buf[1] == '?') {
                ...
                if (end == NULL || *(end + 1) != '\n') {
                    printf("!: invalid format (expected !?string?)\n");
                    continue;
                }
                *end = '\0';
                char *pattern = buf + 2;
                for (int i = cmd_cnt - 1; i >= 0; i--) {
                ...
                if (!found) {
                    printf("!?%s?: no match in history\n", pattern);
                    continue;
                }
            }
            /* !n — execute command by history number */
            else {
                int n = atoi(buf + 1);
                if (n < 1 || n > cmd_cnt) {
                    printf("!%d: no such history entry (1..%d)\n", n, cmd_cnt);
                    continue;
                }
            ...
            if (found) {
                /* Print the expanded command and use it */
                printf("%s", expanded);
                strcpy(buf, expanded);
            }
        }
        ...
        /* Check for background execution: last argument is "&" */
        int background = 0;
        if (argc > 0 && strcmp(arglist[argc - 1], "&") == 0) {
            background = 1;
            arglist[argc - 1] = NULL;
            argc--;
        }

        /* Empty command after removing & */
        if (argc == 0) continue;

        int inner_flag = inner(arglist);
        if (inner_flag) continue;

        result = callCommand(argc, background);
        switch (result) {
